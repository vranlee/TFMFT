...
class Multi_Tracker(object):
    def __init__(self, score_thresh, max_age=32):        
        ...
        self.mid_thresh = (self.high_thresh-self.low_thresh)/2.0
    ...
    def step(self, output_results):
        ...
        for idx in range(scores.shape[0]):
            ...
            if scores[idx] >= self.score_thresh:
            ...
            elif scores[idx] >= self.mid_thresh:
                second_obj = dict()
                ...
                third_obj = dict()
                third_obj["score"] = float(scores[idx])
                third_obj["bbox"] = bboxes[idx, :].cpu().numpy().tolist()
                results_third.append(third_obj)
                results_dict[idx] = third_obj
        ...
        # third association - just as a example of multiple association
        N_third = len(results_third)
        unmatched_tracks_obj_third = list()
        for i in unmatched_tracks:
            #print(tracks[i].keys())
            track = tracks[i]
            if track['active'] == 1:
                unmatched_tracks_obj_third.append(track)
        M_third = len(unmatched_tracks_obj_third)
        unmatched_tracks_third = [t for t in range(M_third)]

        if N_third > 0 and M_third > 0:
            det_box_third = torch.stack([torch.tensor(obj['bbox']) for obj in results_third], dim=0) # N_third x 4        
            track_box_third = torch.stack([torch.tensor(obj['bbox']) for obj in unmatched_tracks_obj], dim=0) # M_third x 4                
            cost_bbox_third = 1.0 - box_ops.generalized_box_iou(det_box_third, track_box_third) # N_third x M_third

            matched_indices_third = linear_sum_assignment(cost_bbox_third)
            unmatched_tracks_third = [d for d in range(M_third) if not (d in matched_indices_third[1])]

            matches_third = [[],[]]
            for (m0, m1) in zip(matched_indices_third[0], matched_indices_third[1]):
                if cost_bbox_third[m0, m1] > 0.8:
                    unmatched_tracks_third.append(m1)
                else:
                    matches_third[0].append(m0)
                    matches_third[1].append(m1)

            for (m0, m1) in zip(matches_third[0], matches_third[1]):
                track = results_third[m0]
                track['tracking_id'] = unmatched_tracks_obj[m1]['tracking_id']
                track['age'] = 1
                track['active'] = 1
                ret.append(track)
        ...
        for k in unmatched_tracks_third:
            track = unmatched_tracks_obj[k]
            if track['age'] < self.max_age:
                track['age'] += 1
                track['active'] = 0
                ret.append(track)
